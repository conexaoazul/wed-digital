...
import ProfissionaisContratados, {
  unCheckNenhum,
} from "../ProfissionaisContratados";
import { normalizeBooleanPayload } from "../../utils/Utils";
...
export default function ConfirmacaoCadastroNoivas() {
  ...
  function salvarDadosComplementares() {
    ...
    if (
      DadosCadastro.luaMel.length === 0 ||
      DadosCadastro.qtdConvidados === 0
    ) {
      messageApi.open({
        type: "warning",
    ...
    api
      .put(
        ...
        normalizeBooleanPayload(DadosCadastro),
      )
      .then(() => {
        setStepCurrent(6);
        atualizarDadosCadastroNoivaWebhook(
          normalizeBooleanPayload(DadosCadastro),
        );
      })
      .catch((error) => {
  ...
  const onNextStep = () => {
    if (stepCurrent === 1) {
      ...
      if (
        DadosCadastro.nomeConjuge !== "" &&
        DadosCadastro.qtdConvidados !== "" &&
        DadosCadastro.luaMel !== ""
      ) {
        setStepCurrent(stepCurrent + 1);
      } else {
        messageApi.open({
          ...
          {stepCurrent === 3 ? (
            <div className="cardPassoAtual">
              ...
              <ProfissionaisContratados
                ...
                onNenhumDeles={handleNenhumDeles}
              />
              <div className="col-12 btnProximoConfirmacao">
                <button
                  className="btn btn-primary"
